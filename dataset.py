...
# ─────────────────────────── Numba JIT kernels ───────────────────────────────
# The hot loops in this file iterate over (B * num_columns) variable-length
# list<int>/list<float> columns, copying values into pre-allocated numpy
# buffers. Rewriting them in Numba drops Python-interpreter overhead and can
# roughly double dataloader throughput (especially with many DataLoader
# workers).  If numba is not available we transparently fall back to the
# original numpy implementations – correctness is identical in both paths.
try:
    from numba import njit, prange  # type: ignore

    _HAS_NUMBA = True

    # When DataLoader spawns many workers, each worker running a parallel
    # numba kernel would oversubscribe the CPU. We therefore restrict numba
    # to a single thread per worker unless the user explicitly overrides it
    # via NUMBA_NUM_THREADS. In the single-worker / main-process case the
    # kernels happily scale across cores.
    if 'NUMBA_NUM_THREADS' not in os.environ:
        # Conservatively allow 4 threads; one-worker benches still benefit,
        # and multi-worker setups stay non-oversubscribed because numba
        # scales down to avail cores anyway.
        try:
            import numba
            numba.set_num_threads(min(4, numba.config.NUMBA_DEFAULT_NUM_THREADS))
        except Exception:
            pass
    ...
    def _nb_pad_int(
        ...
        max_len: int,
        B: int,
        out_padded: np.ndarray,   # int64[B, max_len] (zero-filled)
        out_lengths: np.ndarray,  # int64[B] (zero-filled)
    ) -> None:
        """Pad an Arrow ListArray<int> into a 2D buffer. Values <= 0 -> 0."""
        for i in prange(B):
            s = offsets[i]
            e = offsets[i + 1]
            rl = e - s
    ...
    def _nb_pad_float(
        offsets: np.ndarray,      # int64[B+1]
        values: np.ndarray,       # float32[total]
        max_dim: int,
        B: int,
        out_padded: np.ndarray,   # float32[B, max_dim] (zero-filled)
    ) -> None:
        """Pad an Arrow ListArray<float> into a 2D buffer."""
        for i in prange(B):
            s = offsets[i]
            e = offsets[i + 1]
            rl = e - s
            if rl <= 0:
                continue
            ul = rl if rl < max_dim else max_dim
            for j in range(ul):
                out_padded[i, j] = values[s + j]

    @njit(cache=True, parallel=True, boundscheck=False, fastmath=True)
    def _nb_pad_seq_one_col(
        offsets: np.ndarray,      # int64[B+1]
        values: np.ndarray,       # int64[total]
        max_len: int,
        B: int,
        col_slot: int,
        out_3d: np.ndarray,       # int64[B, n_feats, max_len] (zero-filled on first call)
        out_lengths: np.ndarray,  # int64[B] (running max across columns)
    ) -> None:
        """Pad one sequence column into the ``col_slot`` slice of a 3D buffer
        and update the running per-row length (max over all columns of this
        domain). Values <= 0 -> 0.

        Rows are independent so ``prange`` is safe: each iteration writes to
        a disjoint ``out_3d[i, col_slot, :]`` slice and a distinct
        ``out_lengths[i]`` element.
        """
        for i in prange(B):
            s = offsets[i]
            e = offsets[i + 1]
    ...
    def _nb_pad_ts(
        ...
        """Pad an Arrow ListArray<int64> timestamp column."""
        for i in prange(B):
            s = offsets[i]
            e = offsets[i + 1]
            rl = e - s
...
CALENDAR_TZ_OFFSET_SEC: int = 8 * 3600
CALENDAR_NUM_COLUMNS: int = 9


def _build_calendar_int_feats(timestamps: np.ndarray) -> np.ndarray:
    """Decompose UNIX seconds (UTC) into a 9-column UTC+8 calendar id matrix.

    ``timestamps`` shape: (B,). Returns int64 array of shape (B, 9) whose
    column ranges match the embedding table sizes consumed by the model.
    """
    shifted = timestamps.astype(np.int64, copy=False) + CALENDAR_TZ_OFFSET_SEC
    seconds_in_day = np.mod(shifted, 24 * 3600)
    minute_of_day = (seconds_in_day // 60).astype(np.int64)
    hour_of_day = (seconds_in_day // 3600).astype(np.int64)

    days = shifted.astype('datetime64[s]').astype('datetime64[D]')
    months = days.astype('datetime64[M]')
    years = days.astype('datetime64[Y]')
    day_of_week = np.mod(days.astype(np.int64) + 3, 7).astype(np.int64)
    hour_of_week = (day_of_week * 24 + hour_of_day).astype(np.int64)
    day_of_month = (
        (days - months).astype('timedelta64[D]').astype(np.int64) + 1)
    month_of_year = np.mod(months.astype(np.int64), 12).astype(np.int64) + 1
    day_of_year = (
        (days - years.astype('datetime64[D]')).astype('timedelta64[D]').astype(np.int64) + 1)
    is_weekend = (day_of_week >= 5).astype(np.int64)
    part_of_day = (hour_of_day // 6).astype(np.int64)

    out = np.stack([
        minute_of_day, hour_of_day, day_of_week, hour_of_week,
        day_of_month, month_of_year, day_of_year,
...
class PCVRParquetDataset(IterableDataset):
    ...
    def _pad_varlen_int_column(
        ...
        """
        offsets = np.ascontiguousarray(arrow_col.offsets.to_numpy(), dtype=np.int64)
        values = np.ascontiguousarray(arrow_col.values.to_numpy(), dtype=np.int64)

        padded = np.zeros((B, max_len), dtype=np.int64)
        lengths = np.zeros(B, dtype=np.int64)

        if _HAS_NUMBA:
            _nb_pad_int(offsets, values, max_len, B, padded, lengths)
        else:
            for i in range(B):
    ...
    def _pad_varlen_float_column(
        ...
        offsets = np.ascontiguousarray(arrow_col.offsets.to_numpy(), dtype=np.int64)
        values = np.ascontiguousarray(arrow_col.values.to_numpy(), dtype=np.float32)

        padded = np.zeros((B, max_dim), dtype=np.float32)

        if _HAS_NUMBA:
            _nb_pad_float(offsets, values, max_dim, B, padded)
        else:
            for i in range(B):
                start, end = int(offsets[i]), int(offsets[i + 1])
                raw_len = end - start
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        item_ci = self._col_idx.get('item_id')
        if item_ci is not None:
            item_ids = (batch.column(item_ci).fill_null(0)
                        .to_numpy(zero_copy_only=False).astype(np.int64))
        else:
            item_ids = np.arange(B, dtype=np.int64)
        ...
        result = {
            ...
            'time_feats': torch.from_numpy(
                _build_calendar_int_feats(timestamps)),
        }
        ...
        for domain in self.seq_domains:
            ...
            for ci, slot, vs in side_plan:
                col = batch.column(ci)
                offs = np.ascontiguousarray(col.offsets.to_numpy(), dtype=np.int64)
                vals = np.ascontiguousarray(col.values.to_numpy(), dtype=np.int64)
                col_data.append((offs, vals, vs, ci))

            if _HAS_NUMBA:
                # Numba path: one kernel call per column, writing into the
                # shared 3D buffer and updating `lengths` in place.
                for c, (offs, vals, _vs, _ci) in enumerate(col_data):
                    _nb_pad_seq_one_col(offs, vals, max_len, B, c, out, lengths)
            else:
                for c, (offs, vals, _vs, _ci) in enumerate(col_data):
                    for i in range(B):
                        s = int(offs[i])
            ...
            if ts_ci is not None:
                ...
                ts_offs = np.ascontiguousarray(ts_col.offsets.to_numpy(), dtype=np.int64)
                ts_vals = np.ascontiguousarray(ts_col.values.to_numpy(), dtype=np.int64)
                # Pad timestamps into shape (B, max_len).
                ts_padded = np.zeros((B, max_len), dtype=np.int64)
                if _HAS_NUMBA:
                    _nb_pad_ts(ts_offs, ts_vals, max_len, B, ts_padded)
                else:
                    for i in range(B):
                        s = int(ts_offs[i])
                        e = int(ts_offs[i + 1])
...
def _dataloader_worker_init(_worker_id: int) -> None:
    """DataLoader worker init: pin numba to a single thread.

    Each DataLoader worker is its own process; numba's ``parallel=True``
    kernels would otherwise spawn N_cores threads *per worker*, causing CPU
    oversubscription. The aggregate parallelism across workers already
    saturates the machine, so restricting each worker to one numba thread is
    the right default. Must be a top-level function so the DataLoader can
    pickle it when spawning workers.
    """
    try:
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    if num_workers > 0:
        ...
        _train_kw['worker_init_fn'] = _dataloader_worker_init
