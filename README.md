...
# 【0.833830】taac-infonce优化 相对 taac源代码 —— 新增优化点清单

> 左侧 = `【0.833830】taac-infonce优化`（0.83383 线上版本）
> 右侧 = `taac源代码`（裸 baseline）
>
> 本清单只列 **左侧相对右侧新增** 的优化点，不重复二者共有的组件。
> 每个优化点按 **「解决什么问题 → 怎么做 → 关键实现 → 为什么不与已有组件冗余」** 展开。
...
| 新增点 | 与已有组件的关系 | 是否冗余 |
| --- | --- | --- |
| TargetAttnBias | 加在 cross-attn softmax 之前，alpha 初值 0 | ✅ 不冗余 |
| DIN 残差 | 加在 logits 输入向量（输出残差），位置与 attn bias 不重合 | ✅ 不冗余 |
| CalendarEmbedding | 绝对时间维度，RoPE 是相对维度 | ✅ 不冗余 |
| UserDense 分组 | 是已有通路的内部升级 | ⚠️ 替代关系 |
| AuxProjHead | 独立子空间，不共享 logit head 参数 | ✅ 不冗余 |
| listwise InfoNCE | 主任务仍是 BCE，InfoNCE 是辅助加权项 | ✅ 不冗余 |
| SupCon | 样本级，与 pair 级 InfoNCE 不重叠 | ✅ 不冗余 |
| EMA | 只对 dense，不动 sparse | ✅ 不冗余 |
