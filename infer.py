...
_FALLBACK_MODEL_CFG = {
    ...
    'time_feats_dropout': 0.1,
    'user_dense_grouped': True,
    'use_din': False,
    'din_top_k': 32,
    'din_hidden_mult': 2,
    # Auxiliary contrastive heads. At inference time we always disable
    # them (predict() does not use the projection heads), which keeps
    # state_dict shapes identical to a no-aux training run.
    'use_aux_loss': False,
    'aux_proj_dim': 64,
    # Historical behavior importance weighting (legacy; default off at infer)
    'use_importance_weighting': False,
    'importance_weighting_type': 'cross_attention',
    'importance_dropout': 0.0,
    # 方案 A: Target-aware Cross-Attention Bias. 必须与训练 ckpt 对齐, 否则
    # state_dict 加载会报 unexpected keys (target_bias_modules.* / target_bias_alpha).
    'use_target_attn_bias': False,
...
def load_model_state_strict(
    ...
    Handles the ``_orig_mod.`` prefix that ``torch.compile(model)`` silently
    injects into every key at save time — if the checkpoint was produced
    by a compiled model but inference runs on the uncompiled one, every
    key would otherwise trigger an ``Unexpected key`` error.
    """
    state_dict = torch.load(ckpt_path, map_location=device)

    # ── Strip the ``_orig_mod.`` prefix added by ``torch.compile`` ──
    # Pattern: all keys share the same prefix; so a single check on any
    # key tells us whether the ckpt was saved from a compiled graph.
    sample_key = next(iter(state_dict))
    if sample_key.startswith("_orig_mod."):
        logging.info(
            "Detected '_orig_mod.' prefix in state_dict (produced by "
            "torch.compile at training time); stripping it so the "
            "uncompiled inference model can load the weights.")
        prefix = "_orig_mod."
        prefix_len = len(prefix)
        state_dict = {
            (k[prefix_len:] if k.startswith(prefix) else k): v
...
def _batch_to_model_input(
    ...
    time_feats = device_batch.get(
        'time_feats',
        torch.zeros(B0, 0, dtype=torch.long, device=device))
    return ModelInput(
